...
class Account:
    ...
    def __init__(self, auth_token, csrf, proxy, name):
        ...
        self.session.user_agent = UserAgent
    ...
    def Get_UserBanners(self):
        ...
        with self.session.get(link, timeout=10) as response:
            ...
            for i in data:
                try:
                    peoples.append(i['item']['itemContent']['user_results']['result']['legacy']['profile_banner_url'])
                except:
                    pass
    ...
    def saveImage(self ,image_url):
        ...
        with open(f'ReadyJPGs/{random.randint(10000000, 10000000000000)}.jpg', 'wb') as handler:
    ...
    def GetFullTweetInfo(self, id_):
        ...
        with self.session.get(f'https://twitter.com/i/api/graphql/wNNG8DBB8EaXw1lq4vFWGA/TweetDetail?{payload}', timeout=10) as response:
            ...
            # try:
            #
            #     full_text = response.json()['data']['threaded_conversation_with_injections_v2']['instructions'][0]['entries'][0]['content']['itemContent']['tweet_results']['result']['legacy']['full_text']
            #
            #     # print(full_text[-3:])
            #     if full_text[-3:] == '...':
            #
            #         try:
            #             full_text = full_text[:-3]+ response.json()['data']['threaded_conversation_with_injections_v2']['instructions'][0]['entries'][0]['content']['itemContent']['tweet_results']['result']['legacy']['retweeted_status_result']['result']['note_tweet']['note_tweet_results']['result']['text']
            #         except:
            #             pass
            #
            #     elif full_text[-1] == '…':
            #
            #         try:
            #             full_text = full_text[:1]+ response.json()['data']['threaded_conversation_with_injections_v2']['instructions'][0]['entries'][0]['content']['itemContent']['tweet_results']['result']['legacy']['retweeted_status_result']['result']['note_tweet']['note_tweet_results']['result']['text']
            #         except:
            #             pass
            #
            #     return full_text
...
a = Account(auth_token='bb488b173b4da69f659e37c6b53a3040c7c76295',
            csrf='d6df416356574f07064e1ef7c8383bd9312c505d183b704c710e47b423a31dd422729981e171a8576c0c9a6cc752ef98576200028cc294630d4020448f0d53cb91e0223fb80356b350421ee93e0eac5b',
            proxy='45.152.208.188:8219:eygblzhp:8dkvatehardh',
            name='1')
...
# def ThredTest():
# 
#     data = a.Follow(1501997405755088900)
#     print(data)
# 
# ThredTest()
# 
...
# 
# 
#     # a.GetFullTweetInfo(1646953602370490378)
# 
# 
#     UsedIds = []
#     UsedTweets = []
# 
#     while True:
#         tweets = a.SearchTweets()['statuses']
#         for tweet in tweets:
# 
#             if 'giveaway' in tweet['text'].lower() or 'RT @' == tweet['text'][:4] or tweet['id'] in UsedIds or tweet['text'] in UsedTweets:
#                 continue
# 
#             UsedIds.append(tweet['id'])
#             UsedTweets.append(tweet['text'])
# 
#             fullText, mediaId = a.GetFullTweetInfo(tweet['id'])
#             if fullText == None or 'giveaway' in fullText.lower() or 'thread' in fullText.lower() or 'winner' in fullText.lower() or '#副业' in fullText or fullText.count('@') > 3 or 'airdrop' in fullText.lower() or fullText[1] == '/':
#                 continue
# 
#             if 'https' in fullText[-23:]:
#                 fullText = fullText[:-23]
# 
#             # print(tweet['id'])
#             # print('')
#             print(fullText)
...
# 
#             try:
#                 if last_id - 20000 == ids[0][0]:
#                     cursor.execute("""DELETE FROM posts WHERE id = ?""", (ids[0][0],))
#                     conn.commit()
#             except:
#                 pass
# 
#             cursor.execute(f"""INSERT INTO posts VALUES (?,?,?,?)""", (last_id+1, fullText, False if mediaId == None else True, mediaId))
#             conn.commit()
#             conn.close()
# 
#             time.sleep(3)
